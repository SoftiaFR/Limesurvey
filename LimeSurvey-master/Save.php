...
class Save {
    ...
    function savedcontrol()
    {
        ...
        else
        {
            ...
            $sumExist = Yii::app()->db->createCommand("SELECT count(*) as sum FROM lime_groups WHERE group_name='Récapitulatif' AND sid='".$surveyid."'")
            	->queryRow();

            if ($sumExist['sum'] == 1){
            	$res = Yii::app()->db->createCommand("SELECT count(*) as total FROM ( SELECT count(gid) as nbgroup from lime_questions where sid='".$surveyid."' group by gid ) AS t1")
            		->queryRow();
            	$nbgroup = $res['total'];
            	$saved_control->saved_thisstep = $nbgroup;
            } else {
