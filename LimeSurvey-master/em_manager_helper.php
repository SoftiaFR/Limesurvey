        ...
        */
        private function _UpdateValuesInDatabase($updatedValues, $finished=false)
        {
            ...
            if (count($updatedValues) > 0 || $finished)
            {
                ...
                $sumExist = Yii::app()->db->createCommand("SELECT count(*) as sum FROM lime_groups WHERE group_name='Récapitulatif' AND sid='".$this->sid."'")
                    ->queryRow();

                if ($sumExist['sum'] == 1){
                    $res = Yii::app()->db->createCommand("SELECT count(*) as total FROM ( SELECT count(gid) as nbgroup from lime_questions where sid='".$this->sid."' group by gid ) AS t1")
                        ->queryRow();
                    $nbgroup = $res['total'];
                    $setter[] = dbQuoteID('lastpage') . "=" . $nbgroup;
                } else
                {
                    $setter[] = dbQuoteID('lastpage') . "=" . dbQuoteAll($thisstep);
                }
                    ...
                    else if ($this->surveyOptions['allowsave'] && isset($_SESSION[$this->sessid]['scid']))
                    {
                        ...
                        if ($sumExist['sum'] == 1){
                            $res = Yii::app()->db->createCommand("SELECT count(*) as total FROM ( SELECT count(gid) as nbgroup from lime_questions where sid='".$this->sid."' group by gid ) AS t1")
                                ->queryRow();
                            $nbgroup = $res['total'];
                            SavedControl::model()->updateByPk($_SESSION[$this->sessid]['scid'], array('saved_thisstep'=>$nbgroup));
                        } else {
                            SavedControl::model()->updateByPk($_SESSION[$this->sessid]['scid'], array('saved_thisstep'=>$thisstep));
                        }
