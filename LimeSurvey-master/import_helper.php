    ...
    foreach ($adata as $row)
    {
        ...
        switch($row['class'])
        {
            ...
            case 'SQ':
                ...
                elseif($qtype == ';' && ($row['type/scale'] == 1 || $row['type/scale'] == 0))
                {
                    ...
                    if($row['type/scale'] == 1)
                    {
                        $result_label = Yii::app()->db->createCommand("SELECT lid FROM lime_labelsets WHERE label_name='".$row['other']."'")->queryAll();

                        $insertlabel = "label".$result_label[0]['lid'];

                        $insertdataAttributes=array();
                        $insertdataAttributes['qid'] = $sqid;
                        $insertdataAttributes['attribute'] = "arrayTextToDropdown";
                        $insertdataAttributes['value'] = $insertlabel;
                    
                        $result = QuestionAttribute::model()->insertRecords($insertdataAttributes);
                        if(!$result)
                        {
                            $results['importwarnings'][] = gT("Warning")." : ".gT("Failed to insert default value").". ".gT("Text file row number ").$rownumber;
                            break;
