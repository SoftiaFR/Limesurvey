    ...
    */
    ...
    private function _exporttsv($surveyid)
    {
        ...
        foreach($data as $key=>$row)//foreach($data as $row) //
        {
            if($row[1] == ";" && $row[0]=="Q")
            {
                $questionQid = Yii::app()->db->createCommand("SELECT qid FROM lime_questions WHERE sid=".$data[1][4]." AND title='".$row[2]."'")->queryAll();
                $subQuestionsQids = Yii::app()->db->createCommand("SELECT qid, title FROM lime_questions WHERE sid=".$data[1][4]." AND parent_qid=".$questionQid[0]['qid']." AND scale_id=1")->queryAll();             
                for($zz=2; $zz<5; $zz++)
                {
                    foreach($subQuestionsQids as $subQuestionQid)
                    {
                        if($subQuestionQid['title'] == $data[$key +$zz][2])
                        {
                            $label = Yii::app()->db->createCommand("SELECT value FROM lime_question_attributes WHERE qid=".$subQuestionQid['qid']." AND attribute='arrayTextToDropdown'")->queryAll();
                            $tablabel = explode("label", $label[0]['value']);
                            $labelname = Yii::app()->db->createCommand("SELECT label_name FROM lime_labelsets WHERE lid=".$tablabel[1])->queryAll();
                            $data[$key +$zz][9]=$labelname[0]['label_name'];
                        }
                    }
                }
            }
            if($row[0]=="Q" && $row[1] == "X" &&strpos($row[2], "AAA") == 0)
            {
                unset($data[$key]);
            }
            
            if($row[0]=="G" && $row[2] == "Récapitulatif")
            {
                unset($data[$key]);
            }
        }
