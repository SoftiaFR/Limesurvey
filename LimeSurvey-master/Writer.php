    ...
    */
    final public function write(SurveyObj $oSurvey, $sLanguageCode, FormattingOptions $oOptions, $bOutputHeaders=true)
    {
        ...
        foreach($oSurvey->responses as $response)
        {
            ...
            ////////////////////////////////////////////////////////////////////
            if($oOptions->answerFormat =='long'){
                //echo "<pre>";var_dump($oOptions->selectedColumns);echo "</pre><hr>";
                foreach($oSurvey->fieldMap as $fM_element)
                {
                    $zz=0;
                    foreach($oOptions->selectedColumns as $ligne_selected)
                    {
                        if($fM_element["type"] == ";" && $fM_element['fieldname'] == $ligne_selected)
                        {
                            //echo "<pre>REPONSE"; var_dump($fM_element['fieldname']); echo "</pre><hr>";
                            $real_sqid_aid = explode("_",$fM_element["aid"]);
                            $real_sqid = Yii::app()->db->createCommand("SELECT qid FROM {{questions}} WHERE title='".$real_sqid_aid[1]."' AND scale_id = 1 AND parent_qid=".$fM_element["qid"])->queryRow();
                            $sqid_attribute = Yii::app()->db->createCommand("SELECT attribute, value FROM {{question_attributes}} WHERE qid =".$real_sqid["qid"])->queryRow();
                            if($sqid_attribute["attribute"] == "arrayTextToDropdown")
                            { 
                                $var_lid = explode("label",$sqid_attribute["value"]);
                                $reponse = Yii::app()->db->createCommand("SELECT title FROM {{labels}} WHERE lid =".$var_lid[1]." AND code = '".$elementArray[$zz]."'")->queryRow();
                                $elementArray[$zz]=$reponse["title"];
                            }
                        }
                        $zz++;
                    }
